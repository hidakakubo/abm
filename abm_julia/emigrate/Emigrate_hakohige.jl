...
module Simulation
    ...
    # 色々なseedでアンサンブル平均を取る
    function main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop_array, go_out_rate, accident_rate, born_rate, seeds)
        ...
        for seeds = 1:length(seeds)
            ...
            DataFrame(episode = [], init_male_prop = []) |> CSV.write("./results/parameters.csv")
            for i = eachindex(init_male_prop_array)  # eachindex(A) -> 1:length(A)
                society = SocietyType(firstsize)
                episode = i
                init_male_prop = init_male_prop_array[i]
                ...
                DataFrame(episode = episode, init_male_prop = init_male_prop) |> CSV.write("./results/parameters.csv", append=true)
            end
...
const init_male_prop_array = Vector(0.1:0.1:1)
const go_out_rate = 0
...
open("./results/params.txt", "w") do out
    ...
    println(out, "init_male_prop_array = $(init_male_prop_array)")
...
@time main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop_array, go_out_rate, accident_rate, born_rate, seeds)