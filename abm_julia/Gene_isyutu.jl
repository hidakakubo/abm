...
module Simulation
    ...
    # 色々なseedでアンサンブル平均を取る
    function main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, go_out_rate_array, born_rate, seeds)
        ...
        for seeds = 1:length(seeds)
            ...
            DataFrame(episode = [], go_out_rate = []) |> CSV.write("./results/parameters.csv")
            for i = eachindex(go_out_rate_array)  # eachindex(A) -> 1:length(A)
                society = SocietyType(firstsize)
                episode = i
                go_out_rate = go_out_rate_array[i]
                ...
                DataFrame(episode = episode, go_out_rate = go_out_rate) |> CSV.write("./results/parameters.csv", append=true)
            end
...
const go_out_rate_array = Vector(0:0.1:1)
const born_rate = 0.3
...
open("./results/params.txt", "w") do out
    ...
    println(out, "go_out_rate_array = $(go_out_rate_array)")
    println(out, "born_rate = $(born_rate)")
    println(out, "seeds = $(seeds)")
end

@time main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, go_out_rate_array, born_rate, seeds)