include("Gene_Society.jl")
include("Gene_Decision.jl")
...
module Simulation
    ...
    # シミュレーション実行
    function run_one_episode(society, iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, go_out_rate, born_rate, episode, seeds)
    ...
    # 色々なseedでアンサンブル平均を取る
    function main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time_array, init_male_prop, go_out_rate, born_rate_array, seeds)
        ...
        for seeds = 1:length(seeds)
            ...
            DataFrame(episode = [], kosodate_time = [], born_rate = []) |> CSV.write("./results/parameters.csv")
            for i = eachindex(kosodate_time_array)  # eachindex(A) -> 1:length(A)
                for j = eachindex(born_rate_array)  
                    society = SocietyType(firstsize)
                    episode = (length(born_rate_array) * (i-1)) + j
                    kosodate_time = kosodate_time_array[i]
                    born_rate = born_rate_array[j]
                    run_one_episode(society, iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, go_out_rate, born_rate, episode, seeds)
                    DataFrame(episode = episode, kosodate_time = kosodate_time, born_rate = born_rate) |> CSV.write("./results/parameters.csv", append=true)
                end
            end
...
const cool_time = 10
const old_time = 0
const kosodate_time_array = Vector(0:1:15)
...
open("./results/params.txt", "w") do out
    ...
    println(out, "kosodate_time_array = $(kosodate_time_array)")
...
@time main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time_array, init_male_prop, go_out_rate, born_rate_array, seeds)