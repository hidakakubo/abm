include("kosodate_Society.jl")
include("kosodate_Decision.jl")
...
module Simulation
    ...
    # シミュレーション実行
    function run_one_episode(society, iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, accident_rate, born_rate, episode, seeds)
    ...
    # 色々なseedでアンサンブル平均を取る
    function main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, accident_rate_array, born_rate_array, seeds)
        ...
        for seeds = 1:length(seeds)
            ...
            for i = eachindex(accident_rate_array)  # eachindex(A) -> 1:length(A)
                for j = eachindex(born_rate_array)  
                    ...
                    run_one_episode(society, iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, accident_rate, born_rate, episode, seeds)
...
@time main(iteration, firstsize, Nmax, life_length, cool_time, old_time, kosodate_time, init_male_prop, accident_rate_array, born_rate_array, seeds)